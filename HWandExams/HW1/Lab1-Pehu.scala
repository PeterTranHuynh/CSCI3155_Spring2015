object Lab1 extends jsy.util.JsyApplication {
  ...
  def sqrtErr(c: Double, x0: Double, epsilon: Double): Double = {
    if (x0 < 0 || epsilon <= 0) throw new IllegalArgumentException
    ...
    t match{
      ...
      case Node(l, d, r) =>
        if (n < d) Node(delete(l,n),d, r)
          ...
          Node(l, m, l1)                // In order succession, Replaces integer to be deleted
        }
  ...
  def eval(e: Expr): Double = e match {
    ...
    case Binary(bop, e1, e2) => bop match {
      ...
      case Div => 
        if (eval(e2) == 0 && eval(e1) < 0) Double.NegativeInfinity 
        else if (eval(e2) == 0 && eval(e1) >= 0) Double.PositiveInfinity
