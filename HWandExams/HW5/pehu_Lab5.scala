object Lab5 extends jsy.util.JsyApplication {
  ...
  def mapFirstWith[W,A](f: A => Option[DoWith[W,A]])(l: List[A]): DoWith[W,List[A]] = l match {
    // Returns passed list at the end, nil
    case Nil => doreturn(l)
    // Checks contents in list, starting at head
    case h :: t => f(h) match {
      // if first element doesn't match, recurse/map list with a new head attached to a new tail
      case None => mapFirstWith(f)(t).map((a:List[A]) => (h::a))
      // if there is a match, map the new list element to the tail
  ...
  def castOk(t1: Typ, t2: Typ): Boolean = (t1, t2) match {
    ...
    case (TObj(fields1), TObj(fields2)) => fields1.forall{
      // Checks if a or b are anything, if not, return true
      case(a,b) if (b == None) || (a == None) => true
      // If both are something, go to field two
      case(a,b) => fields2.get(a) match
      {
        // Checks if there is anything again, if not return true
        case None => true
        // If there is something, compare them
    ...
    e match {
      ...
      case Call(e1, args) => typ(e1) match {
        ...
        case tgot @ TFunction(Right((mode,_,tparam)), tret) =>
          ...
          case PName => args.head match
          {
            case e => 
              if (tparam == typ(e)) typ(e) 
              else err(tparam, e1)
          }
          case PVar => args.head match
          {
            case e => 
              if (tparam == typ(e)) typ(e) 
              else err(tparam, e1)
          ...
          {
            case a => 
              if (isLExpr(a) && tparam == typ(a)) tret 
              else err(tparam, e1)
          }
        }
        case tgot => err(tgot, e1)
        ...
        paramse match
        {
          case Left(paramse) =>
            p match
            {
              case Some(a) =>
                val e2 = 
                if (paramse.forall{case(n,_) => (x!= n)} && a!=x)
                  subst(e1) 
                else e1
                return Function(p, Left(paramse), retty, e2)
             case None => 
                val e2 =
                if (paramse.forall{case(n,_) => (x!= n)} )
                  subst(e1) 
                else e1
                return Function(p, Left(paramse), retty, e2)
            }
          case Right(paramse@(mode,s,t)) => 
            p match
            {
              case Some(a) => 
      ...
      case Call(v1, args) if isValue(v1) =>
        ...
        (v1, args) match {
          ...
          case (Function(p, Right((PVar, x1, _)),_, e1), v2 :: Nil) 
            if isValue(v2) => Mem.alloc(v2) map { a => substfun(substitute(e1, Unary(Deref, a), x1), p)}

          case (Function(p, Right((PRef, x1, _)),_, e1), lv2 :: Nil) 
            if isLValue(lv2) => doreturn(substfun(substitute(e1, lv2, x1), p))
